...
def leer_apikey(path="apikey.txt"):
    if not os.path.exists(path):
        ...
        logging.info("API key guardada en apikey.txt")
    try:
        with open(path, "r") as archivo:
            return archivo.read().strip()
    except Exception as e:
        logging.error(f"No se pudo leer la API key: {e}")
        raise
...
def obtener_argumentos():
    ...
    parser.add_argument(
        "-o", "--output", default="reporte.csv", help="Nombre del archivo CSV de salida"
    )
...
def consultar_brechas(correo, api_key):
    ...
    try:
        ...
        logging.error(f"Error de conexión al consultar brechas: {e}")
        raise
...
def consultar_detalle(nombre, api_key):
    ...
    try:
        ...
        logging.error(f"Error de conexión al consultar detalles: {e}")
        raise
...
def generar_csv(nombre_archivo, lista_detalles):
    try:
        ...
        logging.error(f"No se pudo generar el CSV: {e}")
        raise